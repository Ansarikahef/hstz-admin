...
export default function Register() {
  ...
  const onSubmit = async (ev) => {
    ...
    //   //avatar: `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(form.firstName + " " + form.lastName)}&backgroundColor=162D24&textColor=FDFCF8`,
    ...
    // db.save(state);
    // setSubmitting(false);
    // toast.success("Traveller registered", {
    //   description: `${newUser.firstName} ${newUser.lastName} added to the directory${form.isRegisteredByAdmin ? " (admin)" : ""}.`,
    // });
    // navigate("/users");
    const payload = {
      firstName: form.firstName,
      ...
      guardianRelation: form.guardianName ? form.guardianRelation : "",
      gender: form.gender,
      dateOfBirth: form.dateOfBirth, // should be ISO format
      mobileNumber: form.mobileNumber,
      whatsAppNumber: form.whatsAppNumber || form.mobileNumber,
      emailId: form.emailId,
      address: form.address,
      ...
      // ✅ Admin fields
      createdByAdmin: true,
      adminUserId: 1 // replace with logged-in admin ID
    };
  };

  return (
    <div className="min-h-screen w-full bg-[var(--hz-bg)] hz-grain px-4 sm:px-6 py-10" data-testid="register-page">
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] tracking-[0.22em] uppercase text-[var(--hz-text-2)]">User registration</div>
            </div>
          </div>
          <Link to="/login" className="hz-btn-ghost" data-testid="register-back-to-login">
            <ArrowLeft className="size-4" strokeWidth={1.5} /> Back to sign in
          </Link>
        </div>

        <div className="mb-6">
          <div className="hz-label mb-2">New traveller</div>
          <h1 className="hz-heading text-3xl sm:text-4xl font-medium tracking-tight leading-[1.05]">Register a profile</h1>
          <p className="text-[var(--hz-text-2)] mt-2 max-w-xl">
            Capture identity, contact, security, and (optionally) admin notes. Travellers can later be linked to bookings and packages.
          </p>
        </div>
